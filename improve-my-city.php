...
if ( version_compare( PHP_VERSION, '7.3.0', '<' ) ) {
	...
	/**
	 * We are running under PHP 7.3.0
	 * Display an admin notice and do nothing.
	 */

	is_admin() && add_action( 'admin_notices', function ( ) {
		echo ' <div class="error notice"><p> 		
		<strong>The Improve My City plugin requires PHP 7.3.0 or later. Please deactivate the plugin or update your PHP version!</strong>.
		</p></div> ';
	});
}
...
function imc_register_scripts() {
	...
	wp_enqueue_style( 'roboto-font', 'https://fonts.googleapis.com/css?family=Roboto:300i,400,400i,500,500i,700|Roboto+Slab:700' );

	// GOOGLE MAPS with GET OPTIONS
	$gmapOptions = get_option( 'gmap_settings' );
	$gmapAPIkey = $gmapOptions["gmap_api_key"];

	$gmapLanguage = $gmapOptions["gmap_mlang"] !== '' ? $gmapOptions["gmap_mlang"] : 'en';

	$gmapRegion = $gmapOptions["gmap_mreg"] !== '' ? $gmapOptions["gmap_mreg"] : 'GB';
...
function imc_enqueue_admin_scripts($hook) {
	...
	// Load libs in edit custom post page
		...
		if (get_post_type() == 'imc_issues') {
			...
			wp_register_script( 'imc-gutenberg-custom-js', plugin_dir_url( __FILE__ ) . 'js/imc-gutenberg-custom.js', array( 'jquery' ), '1.0', TRUE );
			wp_enqueue_script( 'imc-gutenberg-custom-js' );
